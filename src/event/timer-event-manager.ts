...
export class TimerEventManager {
    ...
    public receivePacket(packet: Packet): void {
        ...
        if (this.lastPacket) {
            ...
            if (this.lastPacket.status === PacketStatus.BOTH_HANDS && packet.status === PacketStatus.STARTING && this.lastPacket.timeInMilliseconds==0) {
            ...
            if (this.lastPacketRunning && (packet.status === PacketStatus.STOPPED || (packet.status === PacketStatus.IDLE && packet.timeInMilliseconds > 0) || packet.status === PacketStatus.BOTH_HANDS || (this.lastPacket.timeInMilliseconds === packet.timeInMilliseconds && packet.timeInMilliseconds > 0))) {
            ...
            // Reset if time reset to zero with packet idle
            if(this.lastPacket.timeInMilliseconds > 0 && packet.timeInMilliseconds == 0 && packet.status== PacketStatus.IDLE)
            {
                this.lastPacketRunning = false
                this.lastPacketReset = true
                this.fire('reset', packet)
